...
bool NavEKF2_core::InitialiseFilterBootstrap(void)
{
    ...
    // ¶¨ÒåÔÚNED×ø±êÏµÏÂµÄµØÇò×Ô×ªµÄÏòÁ¿
...
void NavEKF2_core::UpdateFilter(bool predict)
{
    ...
    startPredictEnabled = predict;       //°ÑÖÜÆÚ¿ªÊ¼µÄflag´ø½øÀ´

    // don't run filter updates if states have not been initialised
    // ×´Ì¬Ã»ÓÐ³õÊ¼»¯³É¹¦µÄ»°¾Í²»ÅÜÂË²¨¸üÐÂµÄ´úÂë
    ...
    // ¶ÁÈ¡IMU¼ÓËÙ¼ÆºÍÍÓÂÝÒÇµÃµ½µÄÊý¾Ý
    readIMUData();

    // Run the EKF equations to estimate at the fusion time horizon if new IMU data is available in the buffer
    // Èç¹ûÐÂµÄIMUÊý¾ÝÓÐÁË£¬ÄÇ¾Írun EKFºËÐÄ·½³Ì³ÌÐò
    if (runUpdates) {
        // Predict states using IMU data from the delayed time horizon
        // ÓÃIMUÊý¾ÝÀ´½øÐÐ×´Ì¬¸üÐÂÔ¤²â¹ý³Ì
        UpdateStrapdownEquationsNED();

        // Predict the covariance growth
        // ¸üÐÂÔ¤²â¹ý³ÌµÄÐ­·½²î¾ØÕóP
        ...
        // ÓÃcompassµÄÊý¾Ý½øÐÐ×´Ì¬Ô¤¹À
        SelectMagFusion();

        // Update states using GPS and altimeter data
        // ÓÃGPSºÍÆøÑ¹¼ÆµÄÊý¾Ý½øÐÐ×´Ì¬Ô¤¹À
        SelectVelPosFusion();

        // Update states using optical flow data
        // ÓÃ¹âÁ÷µÄÊý¾Ý½øÐÐ×´Ì¬Ô¤¹À
        SelectFlowFusion();

        // Update states using airspeed data
        // ÓÃ¿ÕËÙ¼ÆµÄÊý¾Ý½øÐÐ×´Ì¬Ô¤¹À
        SelectTasFusion();
 ...
 *
 * â€œRecursive Attitude Estimation in the Presence of Multi-rate and Multi-delay Vector Measurementsâ€
